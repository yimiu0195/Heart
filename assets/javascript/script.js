        ...
        var Particle = (function() {
            ...
            Particle.prototype.update = function(deltaTime) {
                // vector từ particle đến chuột
                const dx = mouse.x - this.position.x;
                const dy = mouse.y - this.position.y;
                const dist = Math.sqrt(dx * dx + dy * dy);

                const forceRadius = 100; // phạm vi ảnh hưởng
                if (dist < forceRadius && dist > 1) {
                    const strength = (forceRadius - dist) / forceRadius;
                    const angle = Math.atan2(dy, dx);
                    const force = 80 * strength; // độ mạnh của lực

                    // CHỌN HÚT (dấu +) hoặc ĐẨY (dấu -)
                    this.velocity.x += Math.cos(angle) * force * deltaTime;
                    this.velocity.y += Math.sin(angle) * force * deltaTime;
                }

                // cập nhật vị trí như cũ
...
document.addEventListener("mousemove", function(e) {
    const now = Date.now();
    if (now - lastTrailTime > 50) { // mỗi 50ms tạo 1 vệt
        const heart = document.createElement("div");
        heart.className = "mouse-heart";
        heart.style.left = `${e.pageX - 10}px`;
        heart.style.top = `${e.pageY - 10}px`;
        heart.style.transform = `scale(0.8)`; // nhỏ hơn
        document.body.appendChild(heart);
